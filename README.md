# example-dotnet-nuget

> Find vulnerabilities, trace reachability, and audit open-source risk in your **.NET** project.

## About

This repository is a [Bomly](https://bomly.dev) example project for **.NET** with **NuGet**.
...
```bash
bomly scan --url https://github.com/bomly-dev/example-dotnet-nuget --ecosystems dotnet
```
...
```bash
bomly scan --url https://github.com/bomly-dev/example-dotnet-nuget --ecosystems dotnet --enrich --audit
```

### 3. Trace why a dependency exists

Understand every path that pulls `BCrypt.Net-Core` into your graph:

```bash
bomly explain BCrypt.Net-Core --url https://github.com/bomly-dev/example-dotnet-nuget
```
